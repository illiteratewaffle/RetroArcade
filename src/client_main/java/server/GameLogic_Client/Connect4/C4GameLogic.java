...
public class C4GameLogic {
    ...
    private final GameLogic_Client.Connect4.C4Board c4Board;
    private int c4PiecesPlayed;
    private boolean isC4GameOver;
    private int[] C4lastPlayedPosition = new int[2];
    private GameLogic_Client.Connect4.C4Piece currentPlayer = GameLogic_Client.Connect4.C4Piece.RED; //red goes first

    private GameLogic_Client.Connect4.C4Piece c4winner = GameLogic_Client.Connect4.C4Piece.BLANK;
     ...
     */
    public C4GameLogic() {
        this.c4Board = new GameLogic_Client.Connect4.C4Board(6,7);
    ...
    public void updateGameState() {
        for (int j = 0; j < 7; j++) {
            if (c4winner == GameLogic_Client.Connect4.C4Piece.RED) {
                gameState = GameState.P1WIN;
            } else if (c4winner == GameLogic_Client.Connect4.C4Piece.BLUE) {
     ...
     */
    public GameLogic_Client.Connect4.C4Piece getC4CurrentPlayer() {
    ...
    public int getC4ColTopBlank(int col) {
        GameLogic_Client.Connect4.C4Piece[][] board = c4Board.getC4Board();

        for (int row = board.length-1; row >= 0; row--) {
            if (board[row][col] == GameLogic_Client.Connect4.C4Piece.BLANK) {
                return row;
            }
        }
        return -1;
     ...
     */
    public GameLogic_Client.Connect4.C4Piece getC4Winner() {
        return c4winner;
    }
     ...
     */
    public boolean c4DropPiece(int col, GameLogic_Client.Connect4.C4Piece piece) {
        if (isC4ColFull(col)) {
            ...
            if (C4WinCheckerO1.isC4Win(lastPlayed, piece, c4Board.getC4Board())) {
                ...
                isC4GameOver = true;
                c4winner = GameLogic_Client.Connect4.C4Piece.BLANK;
                System.out.println("Game ended in a draw!");
            } else {
                // Swap player turn
                if (currentPlayer == GameLogic_Client.Connect4.C4Piece.RED) {
                    currentPlayer = GameLogic_Client.Connect4.C4Piece.BLUE;
                }
                else if (currentPlayer == GameLogic_Client.Connect4.C4Piece.BLUE) {
                    currentPlayer = GameLogic_Client.Connect4.C4Piece.RED;
                }
            }
            return true;
        }
    ...
    private boolean isC4ColFull(int col) {
        GameLogic_Client.Connect4.C4Piece[][] board = c4Board.getC4Board();
        return board[0][col] != GameLogic_Client.Connect4.C4Piece.BLANK; // checks if the top row is BLANK (empty)
    }
